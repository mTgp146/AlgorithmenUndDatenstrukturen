...
public abstract class AMergesort<E extends Comparable<E>> extends AbstractMergesort<E> {
	...
	protected void mergesort(E[] a, int left, int right) {
		...
		if (left < right) {
			...
			mergesort2(a, left, m); // divide in half and merge the left side
			mergesort2(a, m + 1, right); // divide in half and merge the right side
			merge(a, left, m, right); // merge the values between left and middle + 1
		}
	}

	protected void mergesort2(E[] a, int left, int right) {

		if (left < right) {
			int m = (left + right) / 2; // initialize the middle between left and right
			this.mergesort2(a, left, m); // divide in half and merge the left side
			this.mergesort2(a, m + 1, right); // divide in half and merge the right side
			merge(a, left, m, right); // merge the values between left and middle + 1
		}
	}
	...
	public void merge(E[] a, int left, int m, int right) {
		...
		for (int i = left; i <= right; i++) {
			if ((r > right) || ((l <= m) && (a[l].compareTo(a[r]) <= 0))) { // if the left value is less than the right
																			// value:
